...
Run from the pi-intercom package checkout when testing this local fork without installing it globally:

```bash
SOCKET="${TMPDIR:-/tmp}/pi-intercom-tmux.sock"
SESSION="pi-worker"
ROOT="$PWD"
tmux -S "$SOCKET" new -d -s "$SESSION" -c "$ROOT" "pi --name worker --extension \"$ROOT/index.ts\" --skill \"$ROOT/skills\""
tmux -S "$SOCKET" attach -t "$SESSION"
```
